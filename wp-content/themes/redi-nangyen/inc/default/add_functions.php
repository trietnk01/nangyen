...
function datetimeConverterVn($date,$format_to){
	$result="";
	if (!empty($date) && $date != '0000-00-00 00:00:00' && $date != '0000-00-00') {
		$arrDate    = date_parse_from_format('Y-m-d H:i:s', $date) ;
		$ts         = mktime($arrDate["hour"],$arrDate["minute"],$arrDate["second"],$arrDate['month'],$arrDate['day'],$arrDate['year']);
...
function portfolio_page_template( $template ) {
	...
	if(get_query_var('za_category') != ''){
		$file = get_template_directory() . '/template-03-nangyen-sanpham.php';
		if(file_exists($file)){
			return $file;
		}			
	}
	if(get_query_var('za_weight') != ''){
		$file = get_template_directory() . '/template-03-nangyen-sanpham.php';
		if(file_exists($file)){
			return $file;
		}			
	...
	if(get_query_var('zaproduct') != ''){
		$file = get_template_directory() . '/template-04-nangyen-sanpham-chi-tiet.php';
		if(file_exists($file)){
			return $file;
		}			
	}		
